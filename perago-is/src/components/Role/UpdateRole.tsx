...
import { hideEdit, toggleEdit } from "@/redux/features/roleEditSlice";
...
const UpdateRole = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3000/role/isRoot/${Number(id)}`)
      .then(({ data }: any) => {
        setIsRoot(data);
      });
  }, [id]);
...
const onSubmit = (data: any) => {
  axios
    ...
    .then(() => {
      ...
      disptch(toggleEdit()); // Dispatch the action returned by toggleEdit
    })
    .catch((error) => {
  ...
  useEffect(() => {
    if (Number(id)) {
      axios
        .get(`http://localhost:3000/role/${Number(id)}`)
        .then(({ data }) => {
          setRole(data);
        })
        .catch(({ response: { data } }) => {
  ...
  return (
    <form
      ...
      className="flex flex-col gap-y-3 p-4 border w-full mt-8 shadow bg-white"
    >
        ...
        {!isRoot ? (
          <Controller
            ...
            defaultValue={
              roles.find(
                (singleRole: any) =>
                  Number(singleRole?.value) === role?.parent_id
              )?.value
            }
            render={({ field }) => (
              <Select
